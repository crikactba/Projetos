...
def main():
    ...
    # Título principal 
    st.write("""# RFV (Recência, Frequência e Valor)

    RFV é utilizado para segmentação de clientes baseado no comportamento 
    de compras e agrupamento em clusters parecidos. Utilizando esse tipo de agrupamento podemos realizar 
    ...
    st.sidebar.write("## Por favor Adicione o arquivo!")
    ...
    if (data_file_1 is not None):
        ...
        #df_compras['ID_cliente'] = pd.to_numeric(df_compras['ID_cliente'])
        #df_compras['ID_cliente'] = df_compras['ID_cliente'].astype(int)
        #st.write(df_compras.dtypes)
